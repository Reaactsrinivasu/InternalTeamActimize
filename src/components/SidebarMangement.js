...
import { SidebarDataMangement } from './SidebarDataMangement';
...
const CustomListItemText = ({ primary, icon, ...rest }) => {
    const fontWeight = icon ? 500 : 600;
    const fontSize = icon ? "16px" : "13px";
    const opacity = icon ? 0.9 : 0.9;
    ...
    return (
        <ThemeProvider theme={theme}>
            <ListItem button onClick={() => navigate(item.path)}>
                <ListItemIcon sx={{ color: theme.components.tables.styleOverrides.containedPrimarysidebar.containedPrimarysidebaricon.color, opacity: "0.9" }}>{item.icon} </ListItemIcon>
...
const SidebarMangement = ({ open, setOpen }) => {
    ...
    return (
        <FixedSidebar>
            <Drawer variant="permanent" open={open} sx={{ flexShrink: 0, backgroundColor: theme.components.tables.styleOverrides.containedPrimarysidebar.containedPrimarysidebarheader.backgroundColor, width: drawerWidth, whiteSpace: 'nowrap', zIndex: theme.zIndex.drawer + 1 }}>
                <div style={{ display: 'flex', flexDirection: 'column', height: '90%' }}>
                    ...
                    <StyledSidebarContent sx={{ backgroundColor: theme.components.tables.styleOverrides.containedPrimarysidebar.containedPrimarysidebarheader.backgroundColor, }}>
                        {SidebarDataMangement.map((item, index) => {
                            return (
...
export default SidebarMangement;
