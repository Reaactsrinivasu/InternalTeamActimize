...
const ExpertWiseEmergency = () => {
  ...
  const expertemergencywise = useSelector((state) => state.expertperemergencyData?.data?.data || []);
  ...
  const currentData = Array.isArray(expertemergencywise) ? expertemergencywise.slice(startIndex, endIndex) : [];
  const handlePageChange = (event, value) => {
    setCurrentPage(value);
  };
 const message = expertemergencywise?.length > 0 ? true : false
  return (
        ...
        <Controls.Grid container sx={{ display: 'flex', width: '100%', paddingLeft: '0%' }}>
          <Controls.Grid container item md={12} sx={{ display: 'flex', justifyContent: 'center', backgroundColor: theme.components.tables.styleOverrides.containedPrimarycardpaper.backgroundColor, padding: '35px 0px', borderRadius: '10px', }} >
              ...
              <LoadingComponent />
                ...
                {!message ? (
                    ...
                    {currentData.map((item, index) => (
                      <>
                        <Controls.Grid item xs={5} sx={{
                          backgroundColor: theme.palette.success.main, borderRadius: '10px', margin: '25px 25px 0px 0px', padding: '5px', cursor:'pointer',
                  ...
                  <Controls.Grid container sx={{ marginTop: '30px', justifyContent: 'center',}}>
                    <Controls.ReusablePagination count={Math.ceil(expertemergencywise.length / itemsPerPage)} page={currentPage} onChange={handlePageChange} />
                  </Controls.Grid>
                  </>
                )}
            ...
            <Controls.Grid container item md={3} sx={{ backgroundColor: theme.components.tables.styleOverrides.containedPrimarysidebar.containedPrimarysidebarheader.backgroundColor, boxShadow: '0px 10px 80px rgba(0, 0, 0, 0.1)', padding: '20px 0px', display: 'block', cursor: 'pointer', borderRadius: '10px' }} >
              <Controls.Grid sx={{ padding: '10px 0px 10px 30px', display: 'flex', alignItems: 'center', backgroundColor: theme.palette.error.main, color: theme.palette.success.main }} onClick={() => navigate(`/hr/experts/expertpage/emergencydetails/${id}`)} >
