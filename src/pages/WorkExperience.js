...
const WorkExperience = () => {
  ...
  const dispatch = useDispatch();
  const workExperienceData = useSelector((state) => state.workexpdata?.data?.work_experiences || []);
  const totalRecords = useSelector((state) => state.workexpdata?.data?.total_count || 0);
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, { resetForm }) => {
      if (!editMode) {
        ...
        dispatch(updateWorkExperienceDetailsStart({ id: userInfo.id, values }));
        Controls.toast.success('Data Updated Successfully');
      }
      setTimeout(() => dispatch(loadWorkExperienceDetailsStart()), 500);
      resetForm();
  ...
  const isFormChanged = () => {
    ...
    return Object.keys(formik.values).some((key) => formik.values[key] !== userInfo[key]);
  };
                ...
                <Controls.Box sx={Controls.formBoxStyle}>
                  ...
                  <LocalizationProvider dateAdapter={AdapterDayjs}>
                    <DesktopDatePicker
                      ...
                      value={formik.values.date_of_join}
                      onBlur={formik.handleBlur}
                      onChange={(date) => formik.setFieldValue("date_of_join", date ? date.toString() : '')}
                      inputFormat="DD-MM-YYYY"
                      placeholder="DD-MM-YYYY"
                      type="date"
                      label="Date of Join"
                  ...
                  <LocalizationProvider dateAdapter={AdapterDayjs}>
                    <DesktopDatePicker
                      ...
                      value={formik.values.date_of_end}
                      onBlur={formik.handleBlur}
                      onChange={(date) => formik.setFieldValue("date_of_end", date ? date.toString() : '')}
                      inputFormat="DD-MM-YYYY"
                      placeholder="DD-MM-YYYY"
                      type="date"
                      label="Date of End"
